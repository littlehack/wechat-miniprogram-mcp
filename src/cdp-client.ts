...
export class CDPClient {
  ...
  private debugMain: boolean;
  private connectionMode: ConnectionMode = 'proxy';
  private parsedScripts = new Map<string, ParsedScript>();
  
  // 调试状态管理
  private pausedState: PausedState = {isPaused: false, callFrames: []};
  ...
  private heartbeatTimeout = 10000; // 10秒心跳超时

  // AppService Target 管理（微信小程序特有）
  private appServiceTarget: AppServiceTarget | null = null;
  private appServiceDebuggerEnabled = false;
  ...
  constructor(cdpPort: number = 62000, debugPort: number = 9421, debugMain: boolean = false) {
   ...
   */
  async connect(maxRetries: number = 3, retryDelay: number = 1000): Promise<void> {
   ...
   */
  async connectToDevTools(host: string = '127.0.0.1', port: number = 9222): Promise<void> {
        ...
        try {
          ...
          if (!message.id && message.method) {
            ...
            if (event.method === 'Debugger.scriptParsed' && event.params) {
              const script = event.params as ParsedScript;
   ...
   */
  async evaluateScript(expression: string, contextId?: number, contextName?: string): Promise<any> {
    const params: any = {
      expression,
      returnByValue: true,
      awaitPromise: true,
   ...
   * 获取所有执行上下文
   */
  async getExecutionContexts(): Promise<any[]> {
   ...
   */
  async enableDebuggerAndWaitScripts(waitMs: number = 3000, reset: boolean = false): Promise<ParsedScript[]> {
    if (reset) {
      this.parsedScripts.clear();
    }
    ...
    // 设置脚本解析事件监听器
    const scriptParsedHandler = (params: ParsedScript) => {
   ...
   */
  async getScriptSource(scriptId: string, jscontextId?: string): Promise<string | null> {
    try {
   ...
   */
  async getAllScriptSources(waitMs: number = 3000): Promise<Array<{scriptId: string; url: string; source: string | null}>> {
    try {
      const scripts = await this.enableDebuggerAndWaitScripts(waitMs);
      const results: Array<{scriptId: string; url: string; source: string | null}> = [];
   ...
   * @param query 搜索查询
   */
  async searchInScripts(scriptId: string, query: string): Promise<any[]> {
    try {
   ...
   */
  async getSourceWithCache(scriptId: string, forceRefresh: boolean = false): Promise<SourceCacheEntry | null> {
    // 检查缓存
    if (!forceRefresh) {
   ...
   * 预缓存所有已解析脚本的源码
   */
  async cacheAllSources(): Promise<number> {
    let cached = 0;
  ...
  searchInSourceCache(
    query: string,
    scriptId?: string,
    options: {isRegex?: boolean; maxResults?: number} = {}
  ): SourceSearchResult[] {
    const results: SourceSearchResult[] = [];
    const maxResults = options.maxResults || 100;

    const scriptsToSearch = scriptId
      ? [this.sourceCache.get(scriptId)].filter(Boolean) as SourceCacheEntry[]
   ...
   * 对压缩 JS（单行）也能正确计算列号
   */
  private offsetToLineColumn(entry: SourceCacheEntry, offset: number): {lineNumber: number; columnNumber: number} {
    // 延迟计算行分割
    if (!entry.lines) {
   ...
   * 获取源码缓存统计
   */
  getSourceCacheStats(): {cachedScripts: number; totalSizeBytes: number; compressedCount: number} {
    let totalSize = 0;
   ...
   * 如果为空会自动触发刷新并重试
   */
  async getContexts(retryIfEmpty: boolean = true): Promise<Array<{id: string; name: string}>> {
    try {
      const id = ++this.requestId;
      const command = {
        id,
   ...
   */
  async getAppServiceContextId(): Promise<string | null> {
   ...
   */
  async ensureAppServiceContext(): Promise<boolean> {
   ...
   */
  async setTargetContext(contextName: string | null): Promise<any> {
    try {
      const id = ++this.requestId;
      const command = {
        id,
   ...
   */
  async getContextIdByName(name: string): Promise<number | null> {
    const contexts = await this.getContexts(false);
   ...
   */
  async getTargetContextId(): Promise<number | null> {
    const contexts = await this.getContexts(false);
   ...
   */
  async discoverAppServiceTarget(): Promise<AppServiceTarget | null> {
    try {
      ...
      const appTarget = targets.find((t: any) => {
   ...
   */
  async attachToAppServiceTarget(): Promise<string | null> {
    try {
      // 先尝试发现
   ...
   */
  async sendCommandToAppService(method: string, params: any = {}): Promise<any> {
    if (!this.connected || !this.ws) {
      await this.connect();
    }
   ...
   */
  async enableAppServiceDebugger(): Promise<void> {
    if (!this.connected || !this.ws) {
      await this.connect();
    }
   ...
   */
  async setBreakpointOnAppService(
    scriptId: string,
    lineNumber: number,
   ...
   */
  async setBreakpointOnAppServiceByUrl(
    url: string,
    lineNumber: number,
    columnNumber: number = 0,
    condition?: string
    ...
    if (matched) {
      return this.setBreakpointOnAppService(
        matched.scriptId,
        lineNumber,
        columnNumber,
        condition
   ...
   */
  async enableAppServiceRuntime(): Promise<void> {
    try {
      await this.sendCommand('Runtime.enable');
   ...
   */
  async enableAppServiceNetwork(): Promise<void> {
    try {
   ...
   */
  async enableAppServiceDebuggerAndWaitScripts(waitMs: number = 5000): Promise<ParsedScript[]> {
   ...
   * 包含函数参数、局部变量、this 引用
   */
  async getAppServiceCallStackDetails(): Promise<{
    callFrames: any[];
    reason: string;
    hitBreakpoints?: string[];
  }> {
          ...
          if (scope.object?.objectId) {
            try {
              ...
              const vars = (scopeResult.result || [])
                .filter((p: any) => !p.name.startsWith('__') && p.enumerable)
                .map((p: any) => ({
                  name: p.name,
                  value: p.value?.value ?? p.value?.description ?? `[${p.value?.type}]`,
                  type: p.value?.type,
   ...
   * 返回包含匹配的脚本及其行号
   */
  async searchInAllScripts(query: string): Promise<Array<{
    scriptId: string;
    url: string;
    lineNumber: number;
    lineContent: string;
   ...
   * 等待下一次暂停
   */
  private waitForPaused(timeoutMs: number = 10000): Promise<CallFrame> {
   ...
   * 获取作用域变量
   */
  async getScopeVariables(objectId: string): Promise<Array<{name: string; type: string; value: unknown; description?: string}>> {
    if (!this.connected || !this.ws) {
      await this.connect();
    }
    ...
    const variables: Array<{name: string; type: string; value: unknown; description?: string}> = [];
  ...
  async setBreakpointOnScript(
    ...
    const resolvedLocations: Array<{scriptId: string; lineNumber: number; columnNumber: number}> = [];
   ...
   * 移除所有断点
   */
  async removeAllBreakpoints(): Promise<{removed: number; failed: string[]}> {
  ...
  getNetworkRequestsFiltered(options: {
    ...
    urlFilter?: string;
  } = {}): {requests: NetworkRequest[]; total: number; page: number; pageSize: number} {
  ...
  getConsoleMessagesPaginated(options: {
    ...
    pageIdx?: number;
    types?: string[];
  } = {}): {messages: ConsoleMessage[]; total: number; page: number; pageSize: number} {
  ...
  async clearSiteData(options: {
    ...
    if (options.localStorage !== false || options.sessionStorage !== false) {
      try {
        ...
        `;
        await this.evaluateScript(script);
        if (options.localStorage !== false) cleared.push('localStorage');
        if (options.sessionStorage !== false) cleared.push('sessionStorage');
      } catch (e) {
   ...
   * 获取所有框架
   */
  async getFrames(): Promise<Array<{id: string; url: string; name?: string; parentId?: string}>> {
    if (!this.connected || !this.ws) {
      await this.connect();
    }
    
    const result = await this.sendCommand('Page.getFrameTree');
    const frames: Array<{id: string; url: string; name?: string; parentId?: string}> = [];
    
    const traverse = (frameTree: any) => {
      if (frameTree.frame) {
        frames.push({
...
/** 全局 CDP 客户端实例 */
let globalClient: CDPClient | null = null;
 ...
 */
export function getCDPClient(cdpPort: number = 62000, debugPort: number = 9421, debugMain: boolean = false): CDPClient {
  if (!globalClient) {
