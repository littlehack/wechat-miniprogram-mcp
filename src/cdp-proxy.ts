...
let startedDebugPort: number | null = null;
let startedCdpPort: number | null = null;

/** 小程序是否已连接 */
let miniProgramConnected = false;

/** JS 执行上下文映射：contextId -> { name, ws } */
interface JsContextInfo {
  id: string;
  name: string;
  ws: WebSocket;
}
const jsContextMap = new Map<string, JsContextInfo>();
...
const pendingOps = new Map<number, string>();
...
let targetContextName: string | null = null;
...
let contextRefreshResolvers: Array<() => void> = [];

/**
 * 等待上下文刷新完成（最多等待 waitMs 毫秒）
 * 当 CDP 客户端连接但 jsContextMap 为空时调用
 */
export function waitForContextRefresh(waitMs: number = 5000): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const cleanup = () => {
      ...
      if (idx !== -1) contextRefreshResolvers.splice(idx, 1);
    };
...
function requestContextRefresh(): void {
  if (!miniProgramConnected) return;
 ...
 */
function findContextByName(name: string): JsContextInfo | null {
  for (const info of jsContextMap.values()) {
    if (info.name === name) return info;
  }
  for (const info of jsContextMap.values()) {
    if (info.name.includes(name)) return info;
  }
  return null;
}
 ...
 */
export function setTargetContext(name: string | null): void {
 ...
 * 获取所有已注册的上下文信息
 */
export function getRegisteredContexts(): Array<{id: string; name: string}> {
 ...
 */
function resolveActiveContext(): void {
 ...
 */
function resolveRoutingContext(parsed: any): string {
 ...
 * 检查代理服务器是否已启动
 */
export function isProxyStarted(): boolean {
 ...
 * 检查小程序是否已连接
 */
export function isMiniProgramConnected(): boolean {
 ...
 * @returns 是否连接成功
 */
export function waitForMiniProgram(timeout: number = 60000): Promise<boolean> {
  if (miniProgramConnected) {
    return Promise.resolve(true);
  }
 ...
 */
export function getStartupIssues(): string[] {
 ...
 */
export function ensureServersStarted(debugPort: number, cdpPort: number, debugMain: boolean): void {
  // 如果端口匹配且已启动，跳过
  ...
  if (debugPortBusy && cdpPortBusy) {
    ...
    startupIssues.push(
      `端口 ${debugPort} 和 ${cdpPort} 均被占用，本进程跳过启动（可能存在独立运行的 WMPFDebugger，正在复用其通道）`,
    );
  ...
  if (!debugPortBusy) {
    ...
    startupIssues.push(
      `调试服务器端口 ${debugPort} 已被占用，本进程未能监听。小程序将连接到占用该端口的其他进程，命令可能无法送达`,
    );
  ...
  if (!cdpPortBusy) {
    ...
    startupIssues.push(
      `CDP 代理端口 ${cdpPort} 已被占用，本进程未能监听。CDP 客户端将连接到占用该端口的其他进程`,
    );
 ...
 * @param debugMain 是否输出主进程调试信息
 */
export function startDebugServer(debugPort: number, debugMain: boolean): void {
   ...
   */
  const sendWmpfMessage = (ws: WebSocket, category: string, data: any): void => {
   ...
   * 处理 setupContext 消息
   */
  const handleSetupContext = (data: any, ws: WebSocket): void => {
   ...
   */
  const handleAddJsContext = (data: any, ws: WebSocket): void => {
   ...
   */
  const handleRemoveJsContext = (data: any, ws: WebSocket): void => {
    const contextId = data.jscontext_id;
   ...
   */
  const onMessage = (ws: WebSocket, message: ArrayBuffer): void => {
    debugLogger(
      `[小程序] 收到原始消息 (十六进制): ${bufferToHexString(message)}`,
    );
        ...
        if (typeof payload === 'string' && payload.includes('Debugger.scriptParsed')) {
          try {
            ...
            if (parsed.method === 'Debugger.scriptParsed' && parsed.params) {
              ...
              const isAppServiceScript = 
                scriptUrl.includes('appservice') ||
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.on('message', (msg: ArrayBuffer) => onMessage(ws, msg));
    ws.on('error', (err) => {
       ...
       wss.clients.forEach((client) => {
         if (client.readyState === WebSocket.OPEN) {
           ...
           if (pendingOps.size > 500) {
             ...
             if (oldest !== undefined) pendingOps.delete(oldest);
           }
 ...
 * @param cdpPort CDP 代理服务器端口
 */
export function startCdpProxyServer(cdpPort: number): void {
  ...
  infoLogger(
    `[CDP代理] 调试链接: devtools://devtools/bundled/inspector.html?ws=127.0.0.1:${cdpPort}`,
  );
