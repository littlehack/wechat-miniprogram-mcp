 ...
 */
export const evaluateScript: ToolDefinition = {
  ...
  handler: async (params: {script: string; context?: string}) => {
 ...
 */
export const searchInSources: ToolDefinition = {
  ...
  handler: async (params: {query: string; is_regex?: boolean; script_id?: string; max_results?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
      ...
      // 使用本地缓存搜索
      const results = client.searchInSourceCache(
        params.query,
        params.script_id,
        {
...
export const setBreakpoint: ToolDefinition = {
  ...
  handler: async (params: {script_url: string; line: number; column?: number; condition?: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
      ...
      const breakpointInfo = await client.setBreakpointOnScript(
        targetScriptId,
        lineNumber,
        columnNumber,
        params.condition
 ...
 * 获取调用栈工具
 */
export const getCallStack: ToolDefinition = {
  name: 'get_call_stack',
 ...
 * 监控变量变化工具
 */
export const watchVariable: ToolDefinition = {
  name: 'watch_variable',
  ...
  handler: async (params: {variable_name: string; scope?: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 反混淆代码工具
 */
export const deobfuscateCode: ToolDefinition = {
 ...
 * 暂停/恢复执行工具
 */
export const pauseOrResume: ToolDefinition = {
  ...
  handler: async (params: {action: 'pause' | 'resume'}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 单步执行工具
 */
export const step: ToolDefinition = {
  name: 'step',
  ...
  handler: async (params: {direction: 'over' | 'into' | 'out'}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 */
export const getPausedInfo: ToolDefinition = {
  ...
  handler: async (params: {include_scopes?: boolean; frame_index?: number; max_vars_per_scope?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
        ...
        if (includeScopes && index === frameIndex) {
            ...
            try {
              ...
              frameDetail.this = {
                ...
                properties: (thisResult.result || [])
                  .filter((p: any) => p.enumerable && !p.name.startsWith('__'))
                  .slice(0, maxVars)
                  .map((p: any) => ({
                    name: p.name,
          ...
          if (frame.scopeChain) {
            for (const scope of frame.scopeChain) {
              if (scope.type === 'global') continue; // 跳过全局作用域
          ...
          if (frameDetail.scopes.length > 0) {
            const localScope = frameDetail.scopes.find((s: any) => s.type === 'local');
            if (localScope) {
              frameDetail.functionArgs = localScope.variables.filter((v: any) => 
      ...
      // 查找当前帧对应的脚本信息
      let currentScriptInfo: any = null;
...
export const removeBreakpoint: ToolDefinition = {
  ...
  handler: async (params: {breakpoint_id?: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 */
export const setBreakpointOnText: ToolDefinition = {
 ...
 * 可以访问该帧的局部变量、参数和闭包变量
 */
export const evaluateOnCallFrame: ToolDefinition = {
  ...
  handler: async (params: {expression: string; frame_index?: number; call_frame_id?: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
      ...
      // 在调用帧上评估表达式
      const result = await client.evaluateOnCallFrame(
        callFrameId,
        params.expression,
        {returnByValue: true, generatePreview: true}
      );
 ...
 */
export const searchInScript: ToolDefinition = {
  ...
  handler: async (params: {script_id: string; query: string; is_regex?: boolean; max_results?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 */
export const getSourceRange: ToolDefinition = {
  ...
  handler: async (params: {script_id: string; start_line: number; end_line: number}) => {
    ...
    try {
      ...
      const range = client.getSourceRange(
        params.script_id,
        params.start_line - 1, // 转换为 0-indexed
        params.end_line - 1
      );
