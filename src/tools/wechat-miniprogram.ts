 ...
 */
export const connectCdp: ToolDefinition = {
  name: 'connect_cdp',
  ...
  handler: async (params: {
    cdp_port?: number;
    debug_port?: number;
    inject_frida?: boolean;
    wait_for_miniprogram?: boolean;
    timeout?: number;
 ...
 * 断开 CDP 连接工具
 */
export const disconnectCdp: ToolDefinition = {
  name: 'disconnect_cdp',
 ...
 * 获取微信小程序状态工具
 */
export const getMiniProgramStatus: ToolDefinition = {
 ...
 * 拦截小程序网络请求工具
 */
export const interceptNetworkRequests: ToolDefinition = {
  ...
  handler: async (params: {url_filter?: string; method?: string}) => {
    const client = getCDPClient();
 ...
 */
export const extractCodeBundle: ToolDefinition = {
  ...
  handler: async (params: {save_path?: string; wait_ms?: number; retry_count?: number}) => {
      ...
      for (const script of allScripts) {
        try {
          ...
          for (let i = 0; i < parts.length - 1; i++) {
            if (!current[parts[i]]) current[parts[i]] = {_type: 'directory', _children: {}};
 ...
 * Hook 小程序 API 工具
 */
export const hookMiniProgramApi: ToolDefinition = {
  ...
  handler: async (params: {api_name: string; action: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 列出所有执行上下文工具
 */
export const listContexts: ToolDefinition = {
  name: 'list_contexts',
 ...
 * 切换执行上下文工具
 */
export const switchContext: ToolDefinition = {
  name: 'switch_context',
  ...
  handler: async (params: {context_name: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 获取小程序存储数据工具
 */
export const getStorageData: ToolDefinition = {
 ...
 * 分析小程序加密参数工具
 */
export const analyzeEncryptionParams: ToolDefinition = {
  ...
  handler: async (params: {target_url: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
...
export const getScriptSource: ToolDefinition = {
  ...
  handler: async (params: {script_id?: string; script_url?: string; file_path?: string; start_line?: number; end_line?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
      ...
      if (params.start_line || params.end_line) {
        if (scriptInfo.scriptId) {
          const range = client.getSourceRange(
            scriptInfo.scriptId,
            (params.start_line || 1) - 1,
            (params.end_line || scriptContent.split('\n').length) - 1
          );
          if (range) {
 ...
 */
export const extractPageBusinessLogic: ToolDefinition = {
  ...
  handler: async (params: {
    ...
    try {
      ...
      // 保存每个页面的方法为单独的 JS 文件
      const savedFiles: string[] = [mainFilePath];
      ...
      for (const page of parsed.pages) {
        ...
        if (Object.keys(page.lifeTimes).length > 0) {
          ...
          for (const [name, info] of Object.entries(page.lifeTimes) as [string, {length: number; source: string}][]) {
        ...
        if (Object.keys(allMethods).length > 0) {
          ...
          for (const [name, info] of Object.entries(allMethods) as [string, {length: number; source: string}][]) {
            methodsContent += `// ${name}\n`;
      ...
      const summary = {
        ...
        totalPages: parsed.pages.reduce((sum: number, p: any) => 
          sum + Object.keys(p.methods).length + Object.keys(p.protoMethods).length + Object.keys(p.lifeTimes).length, 0
        ),
 ...
 */
export const searchAndAutoBreak: ToolDefinition = {
  ...
  handler: async (params: {
    ...
    try {
      ...
      // 按脚本 URL 分组统计
      const groupedByScript: Record<string, number> = {};
      ...
      if (autoBreak && allMatches.length > 0) {
        ...
        for (const match of sortedMatches.slice(0, maxBps)) {
          try {
            ...
            const bpInfo = await client.setBreakpointOnAppService(
              match.scriptId,
              match.lineNumber,
              columnNumber
            );
 ...
 */
export const captureAppServiceNetwork: ToolDefinition = {
