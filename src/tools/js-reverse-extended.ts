 ...
 * 列出网络请求工具
 */
export const listNetworkRequests: ToolDefinition = {
 ...
 * 获取请求发起者工具
 */
export const getRequestInitiator: ToolDefinition = {
  ...
  handler: async (params: {reqid: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 */
export const breakOnXhr: ToolDefinition = {
  name: 'break_on_xhr',
  ...
  handler: async (params: {url: string; mode?: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
      ...
      // 尝试在找到的网络请求函数处设置断点
      let autoBreakpointId: string | null = null;
...
export const listScripts: ToolDefinition = {
  ...
  handler: async (params: {filter?: string; pageSize?: number; pageIdx?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 */
export const saveScriptSource: ToolDefinition = {
  ...
  handler: async (params: {scriptId?: string; url?: string; filePath: string}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 列出控制台消息工具
 */
export const listConsoleMessages: ToolDefinition = {
  ...
  handler: async (params: {pageSize?: number; pageIdx?: number; types?: string[]}) => {
    const client = getCDPClient();
    try {
      await client.connect();
 ...
 * 清除站点数据工具
 */
export const clearSiteData: ToolDefinition = {
...
export const clickElement: ToolDefinition = {
  ...
  handler: async (params: {selector: string; index?: number}) => {
    const client = getCDPClient();
    try {
      await client.connect();
