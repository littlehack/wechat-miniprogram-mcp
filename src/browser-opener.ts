 ...
 * @returns 是否已有连接
 */
function hasExistingConnection(cdpPort: number): boolean {
  // 检查是否已经为这个端口打开过
 ...
 * @param cdpPort CDP 端口号
 */
async function openChromeDevTools(url: string, cdpPort: number): Promise<void> {
  try {
    // 如果已经为这个端口打开过，跳过
    if (openedPorts.has(cdpPort)) {
 ...
 */
export function autoOpenDevTools(cdpPort: number, delay: number = 2000): void {
