 ...
 */
export async function fridaServer(projectRoot, debugFrida) {
    ...
    try {
        ...
        const wmpfProcesses = processes.filter((proc) => proc.name === 'WeChatAppEx.exe');
        if (wmpfProcesses.length === 0) {
            throw new Error('[Frida] 未找到 WeChatAppEx.exe 进程，请先打开微信小程序');
        }
        // 获取父进程 PID
        ...
        const wmpfProcess = processes.filter((proc) => proc.pid === wmpfPid)[0];
        // 获取 WMPF 版本号
        ...
        try {
            scriptContent = (await fsPromises.readFile(path.join(projectRoot, 'frida/hook.js'))).toString();
        }
        catch (e) {
        ...
        try {
            configContent = (await fsPromises.readFile(path.join(projectRoot, 'frida/config', `addresses.${wmpfVersion}.json`))).toString();
            ...
            throw new Error(`[Frida] 未找到版本配置文件: ${wmpfVersion}。支持的版本: ${supportedVersions.join(', ')}`);
        }
...
//# sourceMappingURL=frida-inject.js.map